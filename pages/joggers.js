...
function joggers({products}) {
  return (
    <div>
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center ">
          {Object.keys(products).length === 0 && <p>Currently all the joggers are out of stock, new stock comming soon. Stay tuned</p>}
...
export async function getServerSideProps({context}) {
  ...
  if(!mongoose.connections[0].readyState)
   ...
   let products = await Product.find({category:'Joggers'});
   let joggers = {};

   for (let item of products) {
     if (item.name in joggers) {
 
       if(!joggers[item.name].color.includes(item.color) && item.availablity > 0)
         {
           joggers[item.name].color.push(item.color);
         }
       if(!joggers[item.name].size.includes(item.size) && item.availablity > 0)
         {
           joggers[item.name].size.push(item.size);
         }
     } else {
       
       joggers[item.name] = JSON.parse(JSON.stringify(item));
 
       if (item.availablity > 0) {
         joggers[item.name].color = [item.color];
         joggers[item.name].size = [item.size];
        }
        else
        {
         joggers[item.name].color = [];
         joggers[item.name].size = [];
       }
     }
   }
  //  console.log(products)
  return {
    props: {products:JSON.parse(JSON.stringify(joggers))},
  }
}
