      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center ">
            {Object.keys(products).length === 0 && <p>Currently all the hoddies are out of stock, new stock comming soon. Stay tuned</p>}
...
export async function getServerSideProps({context}) {
  ...
  //  console.log(products)
   ...
   for (let item of products) {
     if (item.name in hoddies) {
 
       if(!hoddies[item.name].color.includes(item.color) && item.availablity > 0)
         {
           hoddies[item.name].color.push(item.color);
         }
       if(!hoddies[item.name].size.includes(item.size) && item.availablity > 0)
         {
           hoddies[item.name].size.push(item.size);
         }
     } else {
       
       hoddies[item.name] = JSON.parse(JSON.stringify(item));
